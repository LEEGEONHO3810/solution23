...
<%
	...
	try {
		...
		if(job_order_id.equals("")){
			...
			query += "	, clm_product_count, clm_machine_id , clm_comment, clm_reg_user, clm_reg_datetime, clm_update_user, clm_update_datetime,clm_client_id )";
			query += " values ";
			query += " ('" + SessionCompanyKey + "', '" + jobOrder_id + "', '"+ product_id +"','"+ planned_datetime +"','"+ start_datetime +"','"+ end_datetime +"', '" + product_count + "', '" + machine_id + "' ";
			query += " , '" + comment + "','" + user_id + "', to_char(now(), 'YYYYMMDDHH24MISSMS'::text), '', '','" + client_id + "')";
			ps = conn.prepareStatement(query);
			numRowsUpdated = ps.executeUpdate();
			stmt.close();
			...
			for(int i=0; i<obj.size(); i++) {
				...
				query += "   select sum(cast(x.clm_order_count as numeric)), x.clm_income_date, x.clm_material_order_id, x.clm_material_order_seq ,coalesce (y.useCnt, 0) as useCnt" ;
				query += " 		from tbl_material_order_detail_info x ";
				query += "   		left outer join ( ";
				query += " 				select clm_material_order_id, clm_material_order_seq , sum(cast(clm_material_use_count as numeric)) as useCnt ";
				query += "   			from tbl_material_use_log  ";
				query += " 				where clm_material_id = '"+ material_id +"' ";
				query += "   			and clm_company_key = '" + SessionCompanyKey + "' ";
				query += " 				group by clm_material_order_id, clm_material_order_seq ";
				query += "  		) y on x.clm_material_order_id = y.clm_material_order_id and x.clm_material_order_seq = y.clm_material_order_seq ";
				query += " 		where x.clm_material_id = '"+ material_id +"' ";
				query += " 		and x.clm_income_yn = 'Y' ";
				query += "  	and x.clm_company_key = '" + SessionCompanyKey + "' ";
				query += "  	group by x.clm_income_date , x.clm_material_order_id,x.clm_material_order_seq ,y.useCnt ";
				query += "  	order by x.clm_income_date asc ) t ";
				query += "  where (t.sum - t.useCnt) != 0 ";
				rs2 = stmt2.executeQuery(query);
				System.out.println("+ tbl_material_order_detail_info : " + query);
