...
jQuery('#btn_material_reg').click(function(e){
		var qc_value = '';
		
		$.ajax({
			url : 'material_info_save_proc.jsp',
			data : { 

						'material_id' : material_id,
						'material_name' : $('#txt_material_name').val(),
						'material_main_id ' : $('#txt_material_main_id option:selected').val(),
						'material_sub_id' : $('#txt_material_sub_id option:selected').val(),
						'material_unit' : $('#txt_material_name_unit option:selected').val(),
						'material_comment' : $('#ta_comment option:selected').val(),
						'material_safety_stock' : $('#txt_clm_material_safety_stock').val(),
						'material_comment' : $('#ta_comment').val(),
						'material_code' : $('#txt_material_code_name').val(),
				   },
			type : "POST",
			dataType : "text",
			error : function() {
			...
			success : function(data){
				alert("자재정보 등록 완료");
				$(location).attr("href", 'material_info_list.jsp?');
			}
		});
	});
	...
	// 	if($('#sb_material_gubun option:selected').val() == "0002"){ //자재구분이 원액이라면
	// 		// 비중 입력란 display
	// 		$('#gravityDiv').css('display','block');
	// 	} else {
	// 		$('#gravityDiv').css('display','none');
	// 		$('#txt_material_specific_gravity').val(0);
	// 	}

	// 	if($('#sb_material_gubun option:selected').val() == "0001"){ //자재구분이 원료 라면
	...
	// 	} else if($('#sb_material_gubun option:selected').val() == "0002"){ //자재구분이 원액이라면
	...
	// 	} else if($('#sb_material_gubun option:selected').val() == "0003"){ //자재구분이 부자재 라면
	// 		$('#iseq_1').css('display','none');
	// 		$('#iseq_2').css('display','none');
	// 		$('#iseq_3').css('display','none');
	// 		$('#iseq_4').css('display','none');
	...
	// 	$("#sb_material_type option:eq(0)").prop("selected", true);
	// }
		...
		<div class="table-responsive pmd-card pmd-z-depth">
			...
			<form id="validationForm" action="" method="post" onsubmit="return false;">
				...
				<div class="pmd-card-body">
					<div class="group-fields clearfix row">
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
							...
							<input type="text" class="mat-input form-control" id="txt_material_name" name="txt_material_name" value="">
							</div>
						</div>
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
							<div id="preCode" class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<select id="txt_material_main_id" name="txt_material_main_id" class="select-simple form-control pmd-select2" style="background-color: #ffffff; width:100%; padding:5px;">
									<option <% if(clm_material_main_type_name.equals("")){ %> selected <% } %>>대분류 선택</option>
									<%
									try{
										stmt = conn.createStatement();
										query = " select clm_code_sub_id, clm_code_sub_name from tbl_code_sub_info where clm_company_key = '" + SessionCompanyKey+"' and clm_code_id = '0001' ";
										rs = stmt.executeQuery(query);
										while (rs.next()) {
											String clm_code_sub_id = rs.getString("clm_code_sub_id");
											clm_code_sub_name = rs.getString("clm_code_sub_name");
									%>
										<option value="<%=clm_code_sub_id%>" <% if(clm_material_main_type_name.equals(clm_code_sub_id)){ %> selected <% } %>><%=clm_code_sub_name%></option>
									<%
										}
										stmt.close();
									} catch (Exception e) {
						...
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
							<div id="preCode" class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<select id="txt_material_sub_id" name="txt_material_sub_id"  class="select-simple form-control pmd-select2" style="background-color: #ffffff; width:100%; padding:5px;">
									<option  <% if(clm_material_sub_type_name.equals("")){ %> selected <% } %>>소분류 선택</option>
									<%
									try{
										stmt = conn.createStatement();
										query = " select clm_code_sub_id, clm_code_sub_name from tbl_code_sub_info where clm_company_key = '" + SessionCompanyKey+"' and clm_code_id = '0002' ";
										rs = stmt.executeQuery(query);
										while (rs.next()) {
											String clm_code_sub_id = rs.getString("clm_code_sub_id");
											clm_code_sub_name = rs.getString("clm_code_sub_name");
									%>
										<option value="<%=clm_code_sub_id%>" <% if(clm_material_sub_type_name.equals(clm_code_sub_id)){ %> selected <% } %>><%=clm_code_sub_name%></option>
									<%
										}
										stmt.close();
									} catch (Exception e) {
					...
					<div class="group-fields clearfix row">
						...
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
							<div id="preMaterialName" class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<select id="txt_material_code_name" name="txt_material_code_name" class="select-simple form-control pmd-select2" style="background-color: #ffffff; width:100%; padding:5px;">
									<option <% if(clm_material_code_name.equals("")){ %> selected <% } %>>자재코드 선택</option>
									<%
									try{
										stmt = conn.createStatement();
										query = " select clm_code_sub_id, clm_code_sub_name from tbl_code_sub_info where clm_company_key = '" + SessionCompanyKey+"' and clm_code_id = '0004' ";
										rs = stmt.executeQuery(query);
										while (rs.next()) {
											String clm_code_sub_id = rs.getString("clm_code_sub_id");
											clm_code_sub_name = rs.getString("clm_code_sub_name");
									%>
										<option value="<%=clm_code_sub_id%>" <% if(clm_material_code_name.equals(clm_code_sub_id)){ %> selected <% } %>><%=clm_code_sub_name%></option>
									<%
										}
										stmt.close();
									} catch (Exception e) {
						...
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
							<div class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<input type="number" class="mat-input form-control" id="txt_clm_material_safety_stock" name="txt_clm_material_safety_stock" value="">
							</div>
						</div>
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<div class="form-group pmd-textfield pmd-textfield-floating-label">
									...
									<select id="txt_material_name_unit" name="txt_material_name_unit" class="select-simple form-control pmd-select2" style="background-color: #ffffff; width:100%; padding:5px;">
										<option <% if(clm_material_unit_name.equals("")){ %> selected <% } %>>자재단위 선택</option>
										<%
										try{
											stmt = conn.createStatement();
											query = " select clm_code_sub_id, clm_code_sub_name from tbl_code_sub_info where clm_company_key = '" + SessionCompanyKey + "' and clm_code_id = '0003' ";
											rs = stmt.executeQuery(query);
											while (rs.next()) {
											String clm_code_sub_id = rs.getString("clm_code_sub_id");
											clm_code_sub_name = rs.getString("clm_code_sub_name");
										%>
											<option value="<%=clm_code_sub_id%>" <% if(clm_material_unit_name.equals(clm_code_sub_id)){ %> selected <% } %>><%=clm_code_sub_name%></option>
										<%
											}
											stmt.close();
											conn.close();
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<textarea required class="form-control" id="ta_comment" name="ta_comment" <% if(_code_type_.equals("U")){ %>readOnly style="background-color:#eee;"<% } %>></textarea>
							</div>
						</div>
					</div>
				...
				<div class="pmd-card-actions">
					...
					<a href="javascript:void(0);" id="btn_material_reg" class="btn btn-primary next">저장</a>
