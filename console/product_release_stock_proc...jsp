		...
		try {
				...
				if(release_order_id.equals("")){
					...
					for(int i=0; i<obj.size(); i++) {
						...
						stmt = conn.createStatement();
						query =  "";
						query += " select  clm_joborder_id,clm_product_id,clm_update_datetime,sum(cast(clm_product_count as numeric)) as plusCnt ";
						query += " from tbl_product_produce_info ";
						query += " where clm_product_id = '"+product_id+"' and clm_company_key = '"+SessionCompanyKey+"' ";
						query += " group by clm_joborder_id,clm_product_id,clm_update_datetime ";
						query += " order by clm_update_datetime asc ";
						rs = stmt.executeQuery(query);
						System.out.println("+ tbl_material_order_detail_info : " + query);

						String totalUseCnt = order_count;
						int intUseCnt = Integer.parseInt(totalUseCnt);

							
							String clm_joborder_id = rs.getString("clm_joborder_id");	// 입고번호
							String clm_product_id = rs.getString("clm_product_id"); 	// 입고 seq
							String plusCnt = rs.getString("plusCnt");					 // 생산재고
							String remainCnt = rs.getString("remainCnt"); 				
							...
							System.out.println("===============update tbl_product_stock========================");
