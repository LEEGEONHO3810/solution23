...
<%
		...
		try {
			...
			query += "where 1=1 and x.clm_delete_yn='N' " + strCondition + " ";
			...
			//while (rs.next()) {
				...
				String clm_client_type			 = "";
				String clm_client_name			 = "";
				String clm_client_serial_no		 = "";
				String clm_client_addr		 	 = "";
				String clm_client_ceo		 	 = "";
				String clm_client_phone			 = "";
				String clm_client_tel			 = "";
				String clm_client_fax			 = "";
				String clm_comment				 = "";
				...
				String clm_reg_user		 		 = "";
				String clm_client_address_detail  = "";
%>
<!--content area start-->
...
<!--tab start-->
    <div class="container-fluid full-width-container value-added-detail-page">
		...
		<div class="table-responsive pmd-card pmd-z-depth">
			...
			<input type="hidden" id="txt_client_id" name="txt_client_id" class="form-control" value="<%=_customer_id_ %>">
			...
			<div class="pmd-card pmd-z-depth">
				<div class="pmd-card-body">
					...
					<div class="group-fields clearfix row">
						...
						<div class="form-group pmd-textfield pmd-textfield-floating-label col-lg-4 col-md-4 col-sm-4 col-xs-12">
							...
							<input type="text" class="mat-input form-control" id="txt_ceo_name" name="txt_ceo_name" value="<%=clm_client_ceo %>">
						</div>
						<div class="form-group pmd-textfield pmd-textfield-floating-label col-lg-4 col-md-4 col-sm-4 col-xs-12">
							<label for="first-name">전화번호<span style="font-weight:bold; color:red;">*</span></label>
							<input type="text" class="mat-input form-control" id="txt_client_tel" name="txt_client_tel" value="<%=clm_client_tel %>" oninput="this.value = this.value.replace(/[^0-9,-]/g, '').replace(/(\..*)\./g, '$1');">
						</div>
						<div class="form-group pmd-textfield pmd-textfield-floating-label col-lg-4 col-md-4 col-sm-4 col-xs-12">
							<label for="first-name">핸드폰<span style="font-weight:bold; color:red;">*</span></label>
							<input type="text" class="mat-input form-control" id="txt_client_phone" name="txt_client_phone" value="<%=clm_client_phone %>" oninput="this.value = this.value.replace(/[^0-9,-]/g, '').replace(/(\..*)\./g, '$1');">
						</div>
					</div>
					<div class="group-fields clearfix row">
						<div class="form-group pmd-textfield pmd-textfield-floating-label col-lg-6 col-md-6 col-sm-6 col-xs-12">
							<label for="first-name">사업자등록번호<span style="font-weight:bold; color:red;">*</span></label>
							<input type="text" class="mat-input form-control" id="txt_client_serial_no" name="txt_client_serial_no" oninput="this.value = this.value.replace(/[^0-9,-]/g, '').replace(/(\..*)\./g, '$1');" value="<%=clm_client_serial_no %>" onKeyUp="JavaScript:$.fnc_only_number(this);">
						</div>
						<div class="form-group pmd-textfield pmd-textfield-floating-label col-lg-6 col-md-6 col-sm-6 col-xs-12">
							<label for="first-name">팩스번호</label>
							<input type="text" class="mat-input form-control" id="txt_client_fax" name="txt_client_fax" value="<%=clm_client_fax %>" oninput="this.value = this.value.replace(/[^0-9,-]/g, '').replace(/(\..*)\./g, '$1');">
						</div>
					</div>
					<div class="group-fields clearfix row">
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
							<div class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<input type="text" id="txt_client_addr" name="txt_client_addr" style="cursor:pointer;" class="form-control" value="<%=clm_client_addr %>" readOnly onClick="JavaScript:sample2_execDaumPostcode();">
							</div>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
							<div class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<input type="text" id="txt_client_address_detail" name="txt_client_address_detail" class="form-control" value="<%=clm_client_address_detail %>">
							</div>
						</div>
					</div>
					<div class="group-fields clearfix row">
				...
				<div class="pmd-card-actions">
					...
					<a href="partner_list.jsp?_customer_id_=<%=_customer_id_ %>" class="btn btn-primary next" id="btn_list" name="btn_list">취소</a>
					<!--
					<a href="partner_info_save_proc.jsp?_customer_id_=<%=_customer_id_ %>" class="btn btn-primary next" id="btn_list" name="btn_list">저장</a>
					-->
				</div>
...
<script>
	$(document).ready(function() {
		...
		$.fnc_only_number = function(obj) {
		 	if ($(obj).val() != null && $(obj).val() != '') {
		 		var tmps = $(obj).val().replace(/([0-9]{3})-?([0-9]{2})-?([0-9]{5})/g, '');
		...
		$.fnc_report_data_save = function() {
			...
			if ($("#txt_client_serial_no").val() == '') {
				alert('사업자등록번호를 입력해주시기 바랍니다.');
				return;
			}
			if ($("#txt_user_pw").val() == '') {
				alert('담당자이름을 입력해주시기 바랍니다.');
				return;
			}
			...
			if ($("#txt_client_email").val() == '') {
				alert('이메일을 입력해주시기 바랍니다.');
				return;
			}
			if ($("#txt_client_addr").val() == '') {
				alert('주소를 입력해주시기 바랍니다.');
				return;
			}
			if ($("#txt_client_address_detail").val() == '') {
				alert('상세주소를 입력해주시기 바랍니다.');
				return;
			}
			$.ajax({
				url: 'partner_info_save_proc.jsp',
				...
				success: function(result){
					...
					jQuery.ajax({
						url: 'file_upload_proc.jsp?order_id=' + partner_id,
						data: form_data,
						contentType : false,
						processData : false,
						type:'POST',
						...
						success: function(result){
							...
							$(location).attr("href", 'partner_info_detail.jsp?_customer_id_=' + partner_id)
						}
					});
				}
			});
	...
	function sample2_execDaumPostcode() {
		new daum.Postcode({
			oncomplete: function(data) {
				...
				document.getElementById("txt_client_addr").value = data.zonecode + ' ' + addr + ' ' + extraAddr;
