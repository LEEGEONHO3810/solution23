						...
						<table id="example" class="table pmd-table table-hover table-striped display responsive nowrap" cellspacing="0" width="100%" style="padding:0px; margin:0px;">
							<tr>
								...
								<td style="padding:5px; margin:0px; border:0px solid transparent; text-align: right; width: 70%;">
									<% if(clm_order_state_type.equals("A") || clm_order_state_type.equals("B")){ %>
						...
						<table id="tbl_cost_item_list" class="table pmd-table table-hover table-striped display responsive nowrap" cellspacing="0" width="100%" style="border:1px solid #ccc;">
							<thead>
								<tr>
									...
									<th style="font-size:14px; text-align:center; font-weight:bold; border:1px solid #ccc; background-color: #eee !important; padding: 5px !important;">삭 제</th>
								</tr>
							</thead>
							<tbody>
<%
...
%>
			<tr id="tr_cost_<%=iSubRowCnt %>">
				...
				<td style="border:1px solid #ccc; padding:3px; text-align:center;">
					<a <% if(clm_order_state_type.equals("A") || clm_order_state_type.equals("B")){ %> href="javascript:$.order_cost_delete('<%=clm_cost_order_id %>','<%=clm_cost_order_cost_seq %>');" <% } %> class="btn btn-danger next" style="font-size:8px; padding:10px;" id="btn_cost_delete" <% if(clm_order_state_type.equals("C") || clm_order_state_type.equals("D")){ %> disabled <% } %>>삭제</a>
				</td>
			</tr>
<%
				iSubRowCnt++;
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<table id="example" class="table pmd-table table-hover table-striped display responsive nowrap" cellspacing="0" width="100%" style="padding:0px; margin:0px;">
			<tr>
				...
				<td style="padding:5px; margin:0px; border:0px solid transparent; text-align: right; width: 70%;">
					<% if(clm_order_state_type.equals("A") || clm_order_state_type.equals("B")){ %>
					<button type="button" class="btn btn-primary btn-sm no_print" onClick="JavaScript:$.fnc_file_row_add();">등록</button>
					<% } %>
				</td>
			</tr>
		</table>
		<table id="tbl_file_item_list" class="table pmd-table table-hover table-striped display responsive nowrap" cellspacing="0" width="100%" style="border:1px solid #ccc;">
			<thead>
				<tr>
					...
					<th style="border:1px solid #ccc; font-size:14px; font-weight:bold; text-align:center;  background-color: #eee !important;">삭제</th>
				</tr>
			</thead>
			<tbody>
<%
...
%>
			<tr id="tr_file_<%=iSubRowCnt %>">
				...
				<td style="border:1px solid #ccc; width:10%; padding:3px; text-align:center;">
					<a <% if(clm_order_state_type.equals("A") || clm_order_state_type.equals("B")){ %> href="javascript:$.order_file_delete('<%=clm_file_order_id %>','<%=clm_file_order_file_seq %>');" <% } %> class="btn btn-danger next" style="font-size:8px; padding:10px;" id="btn_file_delete" <% if(clm_order_state_type.equals("C") || clm_order_state_type.equals("D")){ %> disabled <% } %>>삭제</a>
				</td>
			</tr>
<%
				iSubRowCnt++;
...
%>
		...
		<div>
			<a href="estimation_list.jsp" class="btn btn-primary next">목록</a>
			<% if(clm_order_state_type.equals("A")){ %>
				<a href="#" onClick="JavaScript:$.fnc_report_data_save();" class="btn btn-success next" id="btn_order_change" name="btn_order_modify">수정</a>
				<a href="#" onClick="JavaScript:$.fnc_report_state_change('<%=clm_order_id %>', 'C');" class="btn btn-danger next" id="btn_estimation_cancel" name="btn_estimation_cancel">견적취소</a>
				<a href="#" onClick="JavaScript:$.fnc_report_state_change('<%=clm_order_id %>', 'B');" class="btn btn-success next" id="btn_order_change" name="btn_order_change">주문전환</a>
