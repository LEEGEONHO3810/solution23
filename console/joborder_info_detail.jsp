...
String job_order = (request.getParameter("_joborder_id_")==null)?"":request.getParameter("_joborder_id_");
...
try {
			...
			query += " ,x.clm_product_count ,x.clm_start_datetime ,x.clm_end_datetime ,x.clm_complete_yn , x.clm_comment,c.clm_machine_name";
			query += " from tbl_joborder_info x ";
			query += " left outer join tbl_product_info y on x.clm_product_id = y.clm_product_id ";
			query += " left outer join tbl_machine_info c on x.clm_machine_id = c.clm_machine_id ";
...
<script type="text/javascript" language="javascript">
	...
	jQuery(document).ready(function(){
		...
		jQuery('#btn_model_info_reg').click(function(e){
			...
			appendTr += '<td style="text-align:center;"><input type="button" class="btn btn-danger next" onClick="JavaScript:jQuery.income_material_del(\'' + seq_number + '\',\'\');" readonly value="삭제" /></td>';
			...
			$.ajax({
				...
				success : function(data) {
					...
					for(var i=0; i<data.grid_data.length; i++) {
						...
						var useSum = data.grid_data[i].useSum;
						...
						appendTr += '<tr id="income_tr_' + seq_number + '">';
						appendTr += '<td style="padding:2px; border:1px solid #ccc; text-align:right;"><input type="text" class="form-control" style="border:none; background-color:white; cursor:pointer;" id="material_name_' + seq_number + '"  value="'+ clm_material_name +'" readonly/></td>';
						appendTr += '<input type="hidden" id="material_id_' + seq_number + '" value=""></td>';
						appendTr += '<td style="padding:2px; border:1px solid #ccc; text-align:right;"><input type="text" style="text-align:center; border:none; background-color:white; " class="form-control" id="material_main_' + seq_number + '"  value="'+clm_material_main_type_name+'" readonly/></td>';
						appendTr += '<td style="padding:2px; border:1px solid #ccc; text-align:right;"><input type="text" style="text-align:center; border:none; background-color:white; " class="form-control" id="material_stock_sub_' + seq_number + '"  value="'+clm_material_sub_type_name+'" readonly/></td>';
						appendTr += '<td style="padding:2px; border:1px solid #ccc; text-align:right;"><input type="text" class="form-control" style=" border:none;  background-color:white; text-align:right;" id="material_stock_count_' + seq_number + '" value="'+useSum+'" readonly/></td>';
						appendTr += '<td style="padding:2px; border:1px solid #ccc; text-align:right;"><input type="text" class="form-control" style=" border:none;  background-color:white; text-align:center;" id="material_stock_spec_' + seq_number + '" value="'+clm_material_unit_name+'" readonly/></td>';
						appendTr += '<td style="padding:2px; border:1px solid #ccc; text-align:center;"><input type="button" class="btn btn-danger btn-sm" onClick="JavaScript:jQuery.income_material_del(\'' + seq_number + '\',\'\');" value="삭제" disabled=disabled /></td>';
    ...
    <div class="container-fluid full-width-container value-added-detail-page">
		<div>
			...
			<ol class="breadcrumb text-left">
				...
				<li class="active">작업지시서 상세 정보</li>
			</ol><!--breadcrum end-->
		</div>
					...
					<div class="group-fields clearfix row">
						...
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
							<div id="machineCode" class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<input type="text" id="txt_machine_name" value="<%= clm_machine_name %>" class="form-control" readonly>
							</div>
						</div>
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
							<div id="preCode" class="form-group pmd-textfield pmd-textfield-floating-label">
							...
							<table id="tbl_material_income_slave" class="table pmd-table table-hover table-striped display responsive nowrap" cellspacing="0" width="100%">
								<thead>
									<tr>
										...
										<th style="width: 220px !important; text-align: center; ">대분류</th>
										<th style="width: 220px !important; text-align: center; ">소분류</th>
										<th style="width: 130px !important; text-align: center; ">사용 자재 개수</th>
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="form-group pmd-textfield pmd-textfield-floating-label">
								...
								<textarea required class="form-control" id="ta_comment" name="ta_comment"><%= clm_comment %></textarea>
							</div>
						</div>
					</div>
				</div>
				<div class="pmd-card" style="padding-left: 5px;">
					<button type="button" href="javascript:void(0);" class="btn btn-primary btn-sm" id="" name="">수정</button>
					<button type="button" onclick="location.href='joborder_info_list.jsp'" class="btn btn-primary btn-sm" id="btn_list" name="btn_list">취소</button>
				</div>
