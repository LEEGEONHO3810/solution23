					...
					<thead>
						<tr>
							...
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;width: 320px;">자재명</th>
								<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center; width: 140px;" >자재 대분류</th>
								<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center; width: 140px;" >자재 소분류</th>
								<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center; width: 140px;" >자재 단위</th>
								<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center; width: 120px;" >자재 안전재고</th>
								<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center; width: 120px;" >자재 재고</th>
								<th style="border-top:5px solid #666; font: size 14px; font-weight:bold; text-align:center; width: 130px;" >입고 전 자재 수량</th>
						</tr>
					</thead>
					<tbody>
<%
		...
		try {
			...
			query += " 	, x.clm_material_main_type ,coalesce(z1.clm_code_sub_name,'') as clm_material_main_type_name ";
			query += " 	, x.clm_comment, x.clm_material_safety_stock,x.clm_delete_yn,x.clm_reg_user,x.clm_reg_datetime,x.clm_update_user,x.clm_update_datetime ";
			query += " 	, coalesce(x.clm_material_sub_type, '') clm_material_sub_type, coalesce(z2.clm_code_sub_name,'') as clm_material_sub_type_name ";
			query += "  , c.clm_stock_count , y.clm_income_yn , coalesce (sum(cast(y.clm_order_count   as numeric)),0) as order_count, coalesce(z3.clm_code_sub_name,'') as clm_material_unit_name ";
			...
			query += " left outer join tbl_material_order_detail_info y on x.clm_material_id = y.clm_material_id and x.clm_company_key = y.clm_company_key ";
			query += " where x.clm_company_key = '" + SessionCompanyKey + "' and y.clm_income_yn ='N'  ";
			query += " group by x.clm_material_id,z2.clm_code_sub_id,z1.clm_code_sub_name, x.clm_company_key, x.clm_material_code, x.clm_material_name ";
			query += " ,x.clm_material_main_type, x.clm_comment, x.clm_material_safety_stock,x.clm_delete_yn ";
			query += " ,x.clm_reg_user,x.clm_reg_datetime,x.clm_update_user,x.clm_update_datetime ";
			query += " ,clm_material_sub_type, clm_material_sub_type_name , c.clm_stock_count , y.clm_income_yn ,clm_material_unit_name";
			...
			while (rs.next()) {
				...
				if(clm_stock_count.contains(".")){
				 ...
				 if(order_count.contains(".")){
					String[] split_order_count = order_count.split("\\.");
					int_order_count = split_order_count[0];
					float_order_count = split_order_count[1];
				 }else{
					int_order_count = order_count;
				 }
				 System.out.println(int_order_count);

				 order_count = decFormat.format(Integer.parseInt(int_order_count));


				 if(!float_order_count.equals("")){

					order_count = order_count + "." + float_order_count;
...
%>
							<tr id="tr_<%=clm_material_id %>">
								...
								<td style="text-align: right;"><%=order_count %></td>
							</tr>
<%
				iRowCnt++;
			}
		...
		"drawCallback": function( settings ) {
			...
			if(Number(settings._iDisplayLength) >  Number(settings.aiDisplay.length)){
				...
				for(x=0; x<diff; x++){
					...
					appendTr += '	<td data-title="stock"></td>';
					appendTr += '</tr>';
...
<script>
	...
	$(document).ready(function() {
		...
		if(arr_order_id==0) {
			$("#p_alert_msg").html("다운로드 할 입고 현황을 선택해 주십시요.");
			$('#div_alert_modal').modal("show");
			return 0;
		}
		...
		jQuery.ajax({
			url: 'material_current_state_excel_download_proc.jsp',
			type:'POST',
