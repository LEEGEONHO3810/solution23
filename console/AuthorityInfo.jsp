...
<%
	...
	System.out.println("+ ");
	System.out.println("> AuthorityInfo.authority_name : " + authority_name);
	System.out.println("> AuthorityInfo.user_id : " + user_id);
	System.out.println("+ ");
    ...
    <div class="container-fluid full-width-container value-added-detail-page">
		<div>
			<div class="pull-right table-title-top-action">
				...
				-->
				<button class="btn pmd-ripple-effect btn-primary pmd-z-depth" button type="button" onclick="location.href='AuthorityInfo_detail.jsp'">등록</button>
				...
				<table id="main_list" class="table table-striped table-bordered" style="width:100%; border-top:5px solid #666;">
					<thead>
						<tr>
							...
							<th style="font-size:14px; font-weight:bold; text-align:center;">권한명</th>
							<th style="font-size:14px; font-weight:bold; text-align:center;">권한작성자</th>
							<th style="font-size:14px; font-weight:bold; text-align:center;">특이사항</th>
							<th style="font-size:14px; font-weight:bold; text-align:center;">상세정보</th>
							<th style="font-size:14px; font-weight:bold; text-align:center;">사용여부</th>
						</tr>
					</thead>
					<tbody>
<%
		...
		try {
			...
			if(!authority_name.equals("")){
				strCondition += " and (upper(x.clm_authority_name) like upper('%" + authority_name + "%')) ";
			}
			query  = "";
			query = " select x.clm_authority_id , x.clm_authority_name , x.clm_reg_user , x.clm_comment ";
			query += "      , case when x.clm_delete_yn ='N' then '사용' else '미사용' end as clm_delete_yn ";
			query += " from tbl_authority_info x ";
			query += " where 1=1 ";
			query += " 		and clm_delete_yn = 'N' ";
			query += strCondition;
			System.out.println("> AuthorityInfo.q.0 " + query);
			...
			int iRowCnt = 0;
			while (rs.next()) {
				String clm_authority_id = rs.getString("clm_authority_id");
				String clm_authority_name = rs.getString("clm_authority_name");
				...
				clm_comment = clm_comment.replace("'","＇");
				clm_comment = clm_comment.replaceAll("\\\\", "");
				clm_comment = clm_comment.replaceAll("(\r\n|\r|\n|\n\r)", "&#10;");

				// clm_file_download_path   = saveFolderRootLogical + "/" + clm_client_id + ".zip";
%>
							<tr id="tr_<%=clm_authority_name %>">
								<td class="td_first_child" style="text-align:center;">
									<input type="checkbox" class="dt_chk" id="chk_order_id" name="chk_order_id" value="<%=clm_authority_id %>">
								</td>
								<td style="font-size:14px; text-align:left;"><%=clm_authority_name %></td>
								<td style="font-size:14px; text-align:center;"><%=clm_reg_user %></td>
								<td style="font-size:14px; text-align:left;"><%=clm_comment %></td>
								<td style="font-size:14px; text-align:center;"><button type="button" class="btn btn-primary btn-sm" onClick="JavaScript:$.fnc_authority_detail('<%=clm_authority_id %>');" style="padding:2px;"> 상세 </button ></td>
								<td style="font-size:14px; text-align:center;"><%=clm_delete_yn %></td>
							</tr>
<%
				iRowCnt++;
			}
...
$(document).ready(function() {
	...
	$.fnc_authority_detail = function(authority_id) {
		location.href='AuthorityInfo_detail.jsp?authority_id=' + authority_id;
		//location.href='AuthorityInfoRegPop.jsp?authority_id=' + authority_id;
	}
} );
</script>
<script>
	...
	$.fnc_excel_download = function() {
		...
		jQuery.ajax({
			url: 'partner_info_excel_download_proc.jsp',
			type:'POST',
	...
	$.fnc_print = function() {
		...
		var chk_count = 0;
		$("input[name='chk_order_id']:checked").each(function(i) {
			arr_partner_id.push($(this).val());
			str_partner_id += $(this).val() + ',';
			//console.log($(this).val());
		});

		if(arr_partner_id==0) {
			$("#p_alert_msg").html("인쇄 할 업체 정보를 선택해 주십시오.");
			$('#div_alert_modal').modal("show");
			return 0;
		}

		str_partner_id = str_partner_id.substring(0, str_partner_id.length-1);
		
		console.log(str_partner_id);
		$('#ifrm_page').attr('src', 'partner_info_print_proc.jsp?_str_order_id_=' + str_partner_id);
	}
	...
	$.fnc_pdf_download = function() {
		...
		if(arr_partner_id==0) {
			$("#p_alert_msg").html("다운로드 할 업체 정보를 선택해 주십시오.");
			$('#div_alert_modal').modal("show");
			return 0;
		}
