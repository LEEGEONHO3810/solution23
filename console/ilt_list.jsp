...
<!--tab start-->
		...
		<div>
			...
			<h1 class="section-title" id="services">
				<span>잔재 관리 정보 목록</span>
			</h1><!-- End Title -->
			<!--breadcrum start-->
			<ol class="breadcrumb text-left">
				<li><a href="index.jsp">Home</a></li>
				<li class="active">잔재 관리</li>
				<li class="active">잔재 관리 정보 목록</li>
			</ol><!--breadcrum end-->
		</div>
		...
		<div>
			...
			<table id="main_list" class="table table-striped table-bordered" style="width:100%; border-top:5px solid #666;">
				<thead>
					<tr>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:5%;"><input type="checkbox" class="dt_all_chk" id="chk_all_order_id" name="chk_all_order_id" onClick="JavaScript:$.check_all();"></th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:5%;">관리번호</th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:10%;">의뢰업체</th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:25%;">작업명</th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:15%;">이미지</th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:10%;">보관위치</th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:15%;">등록일시</th>
						<th style="font-size:14px; text-align:center; font-weight:bold; width:8%;">등록직원</th>
					</tr>
				</thead>
				<tbody>
<%
...
%>
					<tr id="tr_<%=clm_order_id %>">
						...
						<td data-title="Ticket No" style="font-size:14px; text-align:center;">
							<a href="estimation_info_detail.jsp?_order_id_=<%=clm_order_id %>" style="font-weight:bold; color:#4285f4;"><%=clm_estimation_price %></a>
						</td>
						<!--
						<td data-title="building_height" style="font-size:14px; text-align:right;"><%=clm_building_height %>m</td>
						-->
						<td data-title="building_type" style="font-size:14px; text-align:center;">
							<span class="status-btn blue-bg">
								<%=clm_order_state_type %>
							</span>
						</td>
						<td data-title="reg_datetime" style="font-size:14px; text-align:center;"><%=clm_reg_datetime %></td>
						<!--
						<td class="pmd-table-row-action" style="font-size:14px; text-align:center;">
						...
						<td class="pmd-table-row-action" style="font-size:14px; text-align:center;">
							...
							%>
							<i class="material-icons md-dark pmd-sm" style="color:red; cursor : pointer;" onClick="JavaScript:$.fn_file_download('<%=clm_file_download_path %>');">file_download</i>
							<%
								}
								else {
							%>
						...
						-->
						<!--
						<td class="pmd-table-row-action" style="font-size:14px;">
...
console.log(date);
	...
	$.fnc_search = function() {
		...
		jQuery.ajax({
			...
			success: function(result){
				...
				var order_id = '';
				var tag = '';
				// console.log('> report_item_info_data ' + report_item_info_data);
				...
				jQuery.each(report_item_info_data, function(key, value){
					if(key=='order_data') {
						for(var i=0; i<value.length; i++) {
							tag += '<tr>';
							tag += '	<td data-title="clm_order_id" style="font-size:14px; text-align:center;">';
							...
							tag += '		</a>';
							tag += '	</td>';
							tag += '	<td data-title="Browser Name" style="font-size:14px; text-align:center;">' + value[i].clm_customer_name + '</td>';
							tag += '	<td data-title="building_address" style="padding:0px; font-size:14px;">';
							tag += '		<a href="javascript:void(0);" class="btn pmd-btn-fab pmd-btn-flat pmd-ripple-effect btn-default btn-sm">';
							tag += '			<i class="material-icons md-dark pmd-sm" style="color:grey;">check_circle_outline</i>';
							tag += '		</a>';
							...
							tag += '	<td data-title="building_height" style="font-size:14px; text-align:right;">' + value[i].clm_building_height + 'm</td>';
							tag += '	<td data-title="building_type" style="font-size:14px;">';
							...
							tag += '	<td class="pmd-table-row-action" style="font-size:14px; text-align:center;">';
							tag += '		<a href="JavaScript:$.order_modify_select(\'' + value[i].clm_order_id + '\', \'' + value[i].clm_order_state_type + '\')" class="btn pmd-btn-fab pmd-btn-flat pmd-ripple-effect btn-default btn-sm">';
							tag += '			<i class="material-icons md-dark pmd-sm">edit</i>';
							tag += '		</a>';
							tag += '		<a href="javascript:$.fnc_report_data_delete(\'' + value[i].clm_order_id + '\');" class="btn pmd-btn-fab pmd-btn-flat pmd-ripple-effect btn-default btn-sm">';
							tag += '			<i class="material-icons md-dark pmd-sm">delete</i>';
							tag += '		</a>';
							tag += '	</td>';
							tag += '	<td class="pmd-table-row-action" style="font-size:14px; text-align:center;">';
							tag += '		<a href="<%=clm_file_download_path %>' + value[i].clm_order_id + '.zip" class="btn pmd-btn-fab pmd-btn-flat pmd-ripple-effect btn-default btn-sm">';
							tag += '			<i class="material-icons md-dark pmd-sm" style="color:red;">file_download</i>';
							tag += '		</a>';
							tag += '	</td>';
							tag += '	<td class="pmd-table-row-action" style="font-size:14px;">';
							tag += '		<a href="javascript:$.fnc_report_data_mail(\'' + value[i].clm_order_email + '\');" class="btn pmd-btn-fab pmd-btn-flat pmd-ripple-effect btn-default btn-sm">';
							tag += '			<i class="material-icons md-dark pmd-sm" style="color:red;">domain_verification</i>';
							tag += '		</a>';
							tag += '		<a href="#" data-target="#form-dialog_02" data-toggle="modal" style="font-weight:bold;">';
							tag += '			재발송';
							tag += '		</a>';
							tag += '	</td>';
	...
	$.fnc_excel_download = function() {
		...
		// var order_id				 = $('input[name=chk_order_id]').val();
		// var customer_name			 = $('input[name=txt_customer_name]').val();
		// var building_address		 = $('input[name=txt_building_address]').val();
		// var user_name				 = $('input[name=txt_user_name]').val();
		// var work_from_date			 = $('input[name=txt_work_from_date]').val();
		...
		jQuery.ajax({
			...
			success: function(result){
				...
				jQuery.each(report_item_info_data, function(key, value){
					if(key=='excel_files') {
						for(var i=0; i<value.length; i++) {
							console.log('> ' + i + ' : ' + 'excel/' + value[i].excel_file_name);
							location.href = 'excel/' + value[i].excel_file_name;
						}
					}
				});

				$("#p_alert_msg").html("엑셀 다운로드 성공.");
				$('#div_alert_modal').modal("show");
				loader.off();
			}
		});
