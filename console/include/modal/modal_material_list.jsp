...
<head>
	...
	<script>
		$(document).ready(function() {
			$.fnc_select = function(material_id, material_name, material_code, material_main_type, material_sub_type, material_unit) {
				window.parent.$.fnc_select_to_parent_material('<%=param_seq %>',material_id, material_name, material_code, material_main_type, material_sub_type, material_unit);
			}
		});
	</script>
...
try {
	...
	query += " , coalesce(x.clm_material_sub_type, '') clm_material_sub_type, coalesce(z2.clm_code_sub_name,'') as clm_material_sub_type_name , coalesce(x.clm_material_unit, '') clm_material_unit, coalesce(z3.clm_code_sub_name,'') as clm_material_unit_name ";
...
%>		 
				...
				<tr id="tr_<%=mod_material_id %>">
					...
					<td style="font-size:14px; border:1px solid #ccc; text-align:center;"><%=mod_main_type %></td>
					<td style="font-size:14px; border:1px solid #ccc; text-align:center;"><%=mod_sub_type %></td>
					<td style="font-size:14px; border:1px solid #ccc; text-align:center;">
						<a href="#" style="font-weight:bold; color:#4285f4;  " onClick="JavaScript:$.fnc_select('<%=mod_material_id %>','<%=mod_material_name %>','<%=mod_material_code %>','<%=mod_main_type %>','<%=mod_sub_type %>','<%=mod_material_unit %>');" data-dismiss="modal">
							선택
						</a>
					</td>
				</tr>
