				...
				<table id="main_list" class="table table-striped table-bordered" style="width:100%">
					<thead>
						<tr>
							...
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">입고번호</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">거래처</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">자재명</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">주문일자</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">입고 예정일</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">특이사항</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">주문수량</th>
							<th style="border-top:5px solid #666; font-size:14px; font-weight:bold; text-align:center;">입고상태</th>
						</tr>
					</thead>
					<tbody>
<%
		...
		try {
			...
			query += " select x.clm_material_order_id , x.clm_client_id, y.clm_client_name , x.clm_order_date ";
			query += " , x.clm_due_date, x.clm_all_income_yn , x.clm_comment , z.clm_order_count ";
			query += ", case x.clm_all_income_yn when 'Y' then '입고완료' else '입고전' end clm_status ,c.clm_material_name ";
			query += " 	from tbl_material_order_info x ";
			query += " left outer join tbl_material_order_detail_info z on x.clm_material_order_id = z.clm_material_order_id  and x.clm_company_key = z.clm_company_key ";
			query += " left outer join tbl_client_info y on x.clm_client_id = y.clm_client_id and x.clm_company_key = y.clm_company_key ";
			query += " left outer join tbl_material_info c on z.clm_material_id  = c.clm_material_id and z.clm_company_key = c.clm_company_key ";
			query += " 	where x.clm_delete_yn = 'N' and x.clm_company_key = '" + SessionCompanyKey + "' ";
			query += " 	order by x.clm_material_order_id asc , clm_status; ";
...
%>
				<tr id="tr_<%=clm_material_order_id %>">
					...
					<td style="text-align: center;">
						<a href="javascript:location.href='MaterialIncomeRegPopDetail.jsp?MaterialIncomeSeq=<%=clm_material_order_id %>';" style="color: #3366ff; font-weight:bold;" id="clm_material_order_id" ><%=clm_material_order_id %></a>
					</td>
					...
					<td style="text-align: right7;"><%=clm_order_count %></td>
